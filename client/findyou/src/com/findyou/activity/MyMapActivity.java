...
public class MyMapActivity extends Activity {
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case GetFriendLocationResponse.SHOW_FRIEND:
				GetFriendLocationResponse response = (GetFriendLocationResponse) msg
						.getData()
						.getSerializable(FindyouConstants.RESULT_NAME);
				if (response.getCode() != FindyouConstants.SUCCESS_CODE) {
					showMessage(response.getMsg());
					return;
				}
				locationService.showFriendLocation(friendLocation,
						response.getLatitude(), response.getLontitude());
				break;
			}
		};
	};
	...
	private void showMessage(String message) {
		new AlertDialog.Builder(MyMapActivity.this).setTitle("提示")
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		FindyouApplication application = (FindyouApplication) this
				.getApplication();
		application.setFriendName(friendName);
		application.setFriendPhoneNum(phoneNumber);
		if (getFriendLocationThread == null
				|| !getFriendLocationThread.isAlive()) {
			getFriendLocationThread = new GetFriendLocationThread(phoneNumber,
					mHandler);
	...
	public void locationMe(View view) {
		...
		Toast.makeText(getApplicationContext(),
				result ? "定位请求发送成功!" : "定位请求发送失败!", Toast.LENGTH_SHORT).show();
	}
...
//	}
	...
	private void startLocationService() {
		FindyouApplication app = (FindyouApplication) this.getApplication();
		locationService.start(app, mMapView);
	}
